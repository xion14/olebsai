...
class SellerTransactionExport implements FromCollection, WithHeadings, WithMapping, WithStyles, ShouldAutoSize
{
    protected $startDate;
    protected $endDate;
    protected $status;
    ...
    {
        $query = Transaction::where('seller_id', auth()->user()->seller->id)
                            ->orderBy('id', 'asc');
    ...
    {
        return [
            'No', 'Order ID', 'Tanggal', 'Customer', 'Total (Rp)', 'Status', 'Produk',
        ];
    }
    ...
    {
        return [
            ++$this->rank, // Rank auto-increment mulai dari 1
            $transaction->code,
    ...
    public function styles(Worksheet $sheet)
    {
        return [
            ...
            'A' => ['alignment' => ['horizontal' => 'center']], // No (Rank) tengah
