...
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1 id="title">Customer Withdraw</h1>
...
$(document).ready(function() {
    ...
    var table = $('#withdrawTable').DataTable({
        ...
        // searching: false,
        ajax: {
            url: "{{ route('admin.withdraw.customer') }}",
            type: "GET",
            data: function(d) {
...
$('#confirmWithdrawReject').on('click',function() {
    ...
    $.ajax({
        url: "{{ route('admin.withdraw.customer.reject', 'id') }}".replace('id', id),
        type: "POST",
        data: {
            _token: "{{ csrf_token() }}",
            note: reject_note
...
function processApproval(id, file) {
    ...
    $.ajax({
        url: "{{ route('admin.withdraw.customer.accept', 'id') }}".replace('id', id),
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
