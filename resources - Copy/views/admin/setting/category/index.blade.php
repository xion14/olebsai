        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                var table = registerDatatables({
                    ...
                    url: "{{ route('admin.setting.categories') }}",
                    columns: [{
                            data: 'no',
                            name: 'no'
                        },
                        ...
                        },
                        {
                            ...
                            render: function(data, type, row) {
                                ...
                                let imageUrl = `/uploads/category/${data}`;
                ...
                function switchStatus(id, status) {
                    return new Promise((resolve, reject) => {
                        $.ajax({
                            url: '{{ route('admin.setting.categories.switchStatus') }}',
                            method: 'POST',
                            data: {
                                _token: '{{ csrf_token() }}',
                                id: id,
                ...
                function processForm(mode, id, code, name, image) {
                    return new Promise((resolve, reject) => {
                        ...
                        $.ajax({
                            url: mode === 'create' ?
                                '{{ route('admin.setting.categories.store') }}' :
                                '{{ route('admin.setting.categories.update', ':id') }}'.replace(':id',
                                    id),
                            method: 'POST',
                            data: formData,
                            processData: false,
                ...
                $('#categoryForm').on('submit', function(e) {
                    ...
                    processForm(mode, selectedId, code, name, image)
                        .then(response => {
                            $('.is-invalid').removeClass('is-invalid');
                            $('.invalid-feedback').remove();
                            $('#code').val('');
                ...
                function deleteData(categoryId) {
                    return new Promise((resolve, reject) => {
                        $.ajax({
                            url: '{{ route('admin.setting.categories.destroy', ':id') }}'.replace(':id',
                                categoryId),
                            method: 'POST',
                            data: {
                                _token: '{{ csrf_token() }}',
