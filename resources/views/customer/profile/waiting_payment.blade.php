...
@section('content-setting')
    <div class="col-12 col-lg-8 p-2 p-md-4 p-lg-4 mt-4 mt-lg-2 mt-lg-0 mx-lg-4 border border-0 border-lg-1 rounded-3">
        <div class="w-100 d-flex align-items-center justify-content-between">
            ...
            {{-- <div class="dropdown">
                ...
                <ul class="dropdown-menu">
                    <li><button class="dropdown-item" type="button">30 days ago</button></li>
        ...
        <div id="containerWaitingPayment" class="w-100 mt-3 p-0">
...
@section('script-setting')
    <script type="text/javascript">
        ...
        function getDataWaitingPayment() {
            $.ajax({
                headers: {
                    'X-API-KEY': '{{ env('API_KEY') }}'
                },
                url: '{{ route('api.customer.transaction.index') }}?customer_id={{ session('customer_id') }}&status=3',
                method: 'GET',
                processData: false,
                contentType: false,
                success: function(response) {
                    ...
                    if (response.success == true && response.message == "Success get transactions") {
                        if (dataResponse.length > 0) {
                            dataWaitingPayment = dataResponse;
                            renderWaitingPayment(dataWaitingPayment);
                        } else {
                            dataWaitingPayment = [];
                            renderWaitingPayment(dataWaitingPayment);
                        }
                ...
                error: function(error) {
                    ...
                    console.error("Terjadi kesalahan dalam mengambil waiting payment:", xhr);
                }
            });
        }
            ...
            if (dataResponse.length > 0) {
                $("#containerWaitingPayment").html(
                    dataResponse.map((transaksi, index) => {
                        ...
                        return `
                            <div class="w-100 d-flex flex-column px-2 py-2 px-lg-2 mb-3 border-bottom border-1 gap-4">
                                <strong class="d-inline-block fw-bold mb-2" style="font-size: 16px; line-height: 1.5rem" style="word-break: break-all">
                                    ${transaksi.code}
                                </strong>
                                ...
                                <div class="w-100 d-flex flex-column gap-1">
                                    ...
                                    <div class="w-100 d-flex align-items-center justify-content-end gap-4 mt-1">
                                        ...
                                        <a class="link-offset-2 link-underline link-underline-opacity-0 d-flex align-items-center gap-2 fw-bold payment text-success"
                                            href="#" style="font-size: 14px; line-height: 1.5;" data-id-order="${transaksi.code}">
        ...
        $(document).on("click", ".lihat-detail", function() {
            ...
            detailTransaksi = dataWaitingPayment.find((transaksi) => transaksi.id == transaksiId);
        ...
        $(document).on("click", ".payment", function() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('api.customer.transaction.payment') }}",
                headers: {
                    'X-API-KEY': '{{ env('API_KEY') }}'
                },
                data: {
        ...
        $(document).on("input", ".search-input", function() {
            ...
            let filteredData = dataWaitingPayment.filter(transaksi => {
