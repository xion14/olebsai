            ...
            <div class="section-body">
                <div class="row d-flex flex-column-reverse flex-lg-row gap-4">
                    <div class="col-12 col-lg-7 col-xxl-8">
                        ...
                        <!-- Packing -->
                        <div class="w-100 d-flex flex-column gap-2 px-3 py-3 card mb-4 bg-white">
                            <strong style="font-size: 16px; line-height: 1.5;">Packing</strong>
                            <div class="w-100 mt-3">
                                <div class="row">
                                    <div class="col-md-6">
                                        <label for="shipping-cost-input" class="form-label">Shipping Service</label>
                                        <input type="text" id="shipping-name-input" class="form-control" placeholder="Enter shipping cost" value="{{ $transaction->shipping_name }}">
                                    </div>
                                    <div class="col-md-6">
                                        <label for="shipping-cost-input" class="form-label">Shipping Cost</label>
                                        <input type="number" id="shipping-cost-input" class="form-control" placeholder="Enter shipping cost" value="{{ $transaction->shipping_cost }}">
                                    </div>
                                    <div class="col-md-12 text-right mt-2">
                                        <button class="btn btn-sm btn-primary text-white" id="save-shipping-info">
    ...
    <script>
        ...
        `);
            ...
            function saveToApi(id, name, amount) {
                $.ajax({
                    url: '/admin/transactions/other-cost/update/' + id,
                    method: 'PUT',
                    headers: {
            ...
            function deleteFromApi(id, row) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '/admin/transactions/other-cost/delete/' + id,
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}' // Tambahkan token CSRF dengan benar
                            },
            ...
            $('#save-shipping-info').on('click',function() {
                let shipping_name = $('#shipping-name-input').val();
                let shipping_cost = $('#shipping-cost-input').val();

                $.ajax({
                    url: '/admin/transactions/other-cost/save-shipping', // Ganti dengan endpoint API kamu
                    method: 'POST',
            ...
            $('#submit-cost').on('click', function() {
                ...
                $.ajax({
                    url: '/admin/transactions/other-cost/submit', // Ganti dengan endpoint API kamu
                    method: 'POST',
                    contentType: 'application/json',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}' // Tambahkan token CSRF dengan benar
            ...
            $('#confirm-transaction').click(function() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('admin.transactions.confirm', ['id' => $transaction->id]) }}",
                            type: "POST",
                            data: {
                                _token: "{{ csrf_token() }}",
                                transaction_id: "{{ $transaction->id }}"
                            ...
                            success: function(response) {
                                Swal.fire({
                                    ...
                                    window.open('https://wa.me/62{{ $transaction->seller->phone }}?text=Pesanan%20anda%20sudah%20dikonfirmasi,%20silahkan%20lanjutkan%20pembayaran%20pada%20{{ url('/waiting-payment') }}', '_blank');
                                    window.location.href = '/admin/transactions/waiting_payment';
                                });
                            },
                            error: function(xhr) {
                                Swal.fire({
            ...
            $('#reject-transaction').click(function() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('admin.transactions.reject', ['id' => $transaction->id]) }}",
                            type: "POST",
                            data: {
                                _token: "{{ csrf_token() }}",
                                transaction_id: "{{ $transaction->id }}",
                            ...
                            success: function(response) {
                                Swal.fire({
                                    ...
                                    window.open('https://wa.me/62{{ $transaction->seller->phone }}?text=pesanan%20{{ $transaction->code }}%20telah%20di%20reject%20oleh%20admin', '_blank');
                                    location.reload();
                                });
                            },
                            error: function(xhr) {
